...
public class Waves : MonoBehaviour
{
    ...
    {
        // scale factor (if scale is not 1 always) and position in local space
    ...
    private int[] GenerateTries()
    {
        ...
        // two triangles are equal to one tile
        for (int x = 0; x < Dimension; x++)
            for (int z = 0; z < Dimension; z++)
            {
    ...
    // UVs are texture mapped to the actual wireframe (texture have coordinates (x, z) and map to each pixel in the wireframe)
    private Vector2[] GenerateUVs()
    {
        ...
        for (int x = 0; x <= Dimension; x++)
        {
            for (int z = 0; z <= Dimension; z++)
            {
                // alternate between from every vector to the next:
        ...
        for (int x = 0; x <= Dimension; x++)
        {
            for (int z = 0; z <= Dimension; z++)
            {
                ...
                for (int o = 0; o < Octaves.Length; o++)
                {
                    ...
                    {
                        // add a vector of perlin noise to wave (perlin gives a value between 0 and 1)
                    ...
                    {
                        // add a vector of perlin noise multiplied by time and subtracting 0.5 to put wave at y = 0 if not alternating
                        var perl = Mathf.PerlinNoise((x * Octaves[o].scale.x + Time.time * Octaves[o].speed.x) / Dimension, (z * Octaves[o].scale.y + Time.time * Octaves[o].scale.y) / Dimension) - 0.5f;
    ...
    [Serializable]
    // structure to create overlaps for waves
